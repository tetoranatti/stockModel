...
class DualStreamGRU_v6_Slim5(nn.Module):
    def __init__(self, stock_dim=5, macro_dim=5, hidden_dim=24, num_heads=2, num_classes=3):
        ...
        self.cross_attn = CrossAttentionBlock(hidden_dim, num_heads=num_heads, dropout=0.2)
...
def sync_latest_csv():
    ...
    if files:
        ...
        print(f"[+] 最新スクリーニングCSVを自動同期しました: {os.path.basename(latest)}")
...
def main():
    print("=" * 85)
    print("【v5スイングモデル 動的レジーム制御型スクリーニング (悪地合い対応モード)】")
    print("=" * 85)
    ...
    print(f"\n[★] 現在の相場環境認識 (マクロレジーム):")
    print(f"  - ガンマ状態: {'【ネガティブガンマ (急変警戒・ボラ拡大)】' if is_negative_gamma else '【ポジティブガンマ (安定推移)】'} (乖離: {gamma_dist:.3f})")
    print(f"  - 壁の位置関係: Put壁 [{latest_macro['put_oi_wall']:.0f}] <--- ({wall_ratio*100:.1f}%) ---> Call壁 [{latest_macro['call_oi_wall']:.0f}]")
    print(f"  - 空売り比率: {short_ratio:.1f}% | 適用基本しきい値: {base_threshold:.3f}\n")
    ...
    for i, t in enumerate(tickers):
        try:
            ...
            # =============================================================
            # 動的レジーム適合型 判定エンジン (悪地合い対応モード)
            # =============================================================
            action = "⏸️ WAIT"
            gate_reason = "見送り"

            if is_negative_gamma:
                # 1. 指数連動安のリスクが高い高ベータ順張りは排除
                if beta >= 1.0:
                    action = "⏸️ WAIT"
                    gate_reason = f"地合い連動安警戒(β={beta:.2f})"
                
                # 2. 避難先ディフェンシブ (低ベータ × 損切極小 × 商い維持)
                elif beta <= 0.45 and (p_win > p_stop * 1.15) and (p_win >= 0.320) and (vol_ratio >= 0.9):
                    action = "🛡️ DEFENSIVE BUY"
                    gate_reason = f"避難買い(低β={beta:.2f}, 期待比={p_win/p_stop:.2f})"
                
                # 3. 逆張り自律反発候補 (高スコア × 出来高急増)
                elif (p_win >= 0.350) and (p_win > p_stop) and (vol_ratio >= 1.1):
                    action = "⚡ REVERSAL BUY"
                    gate_reason = f"自律反発初動(勝率{p_win*100:.1f}%, 出来高{vol_ratio:.2f}x)"
                
                # 4. 悪地合い中の監視リスト (ボーダーライン)
                elif (p_win >= 0.330) and (p_win > p_stop):
                    action = "👀 WATCH"
                ...
                elif (p_win >= base_threshold - 0.020) and (p_win > p_stop * 1.1) and (vol_ratio >= 0.8):
                    action = "👀 WATCH"
            ...
            candidates.append({
                "ticker": t,
                "price": curr_close,
                "target_2atr": round(curr_close + 2.0 * curr_atr, 1),
                "stop_1atr": round(curr_close - 1.0 * curr_atr, 1),
    ...
    # ソート順: BUY/DEFENSIVE/REVERSAL を最上位、次に WATCH、最後に WAIT
    priority_map = {"🎯 BUY": 1, "🛡️ DEFENSIVE BUY": 2, "⚡ REVERSAL BUY": 3, "👀 WATCH": 4, "⏸️ WAIT": 5}
    res_df['priority'] = res_df['action'].map(priority_map)
    res_df = res_df.sort_values(by=["priority", "prob_win"], ascending=[True, False]).drop(columns=['priority'])

    res_df.to_csv(OUTPUT_CSV, index=False, encoding='utf-8-sig')

    print("\n" + "=" * 90)
    print("【動的スクリーニング結果 (推奨・監視ピックアップ)】")
    print("=" * 90)
    
    # ピックアップ銘柄（BUY または WATCH）があればそれを表示、なければ上位15件
    pickups = res_df[res_df['action'] != "⏸️ WAIT"]
    if not pickups.empty:
        print(pickups.head(20)[['ticker', 'price', 'target_2atr', 'stop_1atr', 'prob_win', 'beta', 'vol_ratio', 'action', 'reason']].to_string(index=False))
    else:
        print(res_df.head(15)[['ticker', 'price', 'target_2atr', 'stop_1atr', 'prob_win', 'beta', 'vol_ratio', 'action', 'reason']].to_string(index=False))
