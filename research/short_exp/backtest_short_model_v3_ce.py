"""空売りモデルv3(決済理由ラベル+交差エントロピー)のバックテスト(2026-09-24追加)。
train_short_model_v3_ce.pyが保存したチェックポイントを読み込むだけで、v2
(risk_adjusted_blend+NearPairRankingLossRegimeAware)と特徴量集合(SHORT_STOCK_COLS_V2)は
揃えたまま、ラベル・損失関数だけの効果を比較する。

実行方法:
    python research/short_exp/backtest_short_model_v3_ce.py
"""
import sys
import os
...
from train_short_model_v3_ce import SHORT_MODEL_CACHE_DIR_V3
...
def main():
    ...
    print(f"[+] backtest_pool(空売りv3 CE)={len(pool_test)}")

    models = []
    for seed in m.SEEDS:
        ckpt = os.path.join(SHORT_MODEL_CACHE_DIR_V3, f"state_{seed}.pt")
        if not os.path.exists(ckpt):
            raise FileNotFoundError(
                f"{ckpt}が見つかりません。先にtrain_short_model_v3_ce.pyを実行してください。"
            )
    ...
    if m.TICKER_TO_CLUSTER:
        print("\n" + "=" * 90)
        print(f"【TOPN別PF分析(空売りモデルv3 CE、クラスタ制約 max_per_cluster={m.MAX_PER_CLUSTER})】")
        print("=" * 90)
