"""空売りモデルv7(|ret_pct差|重み付けペアワイズ)のバックテスト(2026-09-24追加)。
EV重みは2.0:1.0(v2と同一)なのでscore列の再計算は不要。

実行方法:
    python research/short_exp/backtest_short_model_v7_magnitude_weighted.py
"""
import sys
import os
...
from train_short_model_v7_magnitude_weighted import SHORT_MODEL_CACHE_DIR_V7
...
def main():
    ...
    print(f"[+] backtest_pool(空売りv7)={len(pool_test)}")

    models = []
    for seed in m.SEEDS:
        ckpt = os.path.join(SHORT_MODEL_CACHE_DIR_V7, f"state_{seed}.pt")
        if not os.path.exists(ckpt):
            raise FileNotFoundError(
                f"{ckpt}が見つかりません。先にtrain_short_model_v7_magnitude_weighted.pyを実行してください。"
            )
    ...
    if m.TICKER_TO_CLUSTER:
        print("\n" + "=" * 90)
        print(f"【TOPN別PF分析(空売りモデルv7、クラスタ制約 max_per_cluster={m.MAX_PER_CLUSTER})】")
        print("=" * 90)
