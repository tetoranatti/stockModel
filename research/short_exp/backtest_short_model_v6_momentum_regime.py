"""空売りモデルv6(モメンタムレジーム別ペアワイズ損失)のバックテスト(2026-09-24追加)。
EV重みは2.0:1.0(v2と同一、modules/model_inference.pyのハードコードと一致)なので
score列の再計算は不要——v2/v3/v4と同じ標準経路でそのまま評価できる。

実行方法:
    python research/short_exp/backtest_short_model_v6_momentum_regime.py
"""
import sys
import os
...
from train_short_model_v6_momentum_regime import SHORT_MODEL_CACHE_DIR_V6
...
def main():
    ...
    print(f"[+] backtest_pool(空売りv6)={len(pool_test)}")

    models = []
    for seed in m.SEEDS:
        ckpt = os.path.join(SHORT_MODEL_CACHE_DIR_V6, f"state_{seed}.pt")
        if not os.path.exists(ckpt):
            raise FileNotFoundError(
                f"{ckpt}が見つかりません。先にtrain_short_model_v6_momentum_regime.pyを実行してください。"
            )
    ...
    if m.TICKER_TO_CLUSTER:
        print("\n" + "=" * 90)
        print(f"【TOPN別PF分析(空売りモデルv6、クラスタ制約 max_per_cluster={m.MAX_PER_CLUSTER})】")
        print("=" * 90)
