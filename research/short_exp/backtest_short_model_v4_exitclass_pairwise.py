"""空売りモデルv4(決済理由ラベル+ペアワイズランキング損失)のバックテスト(2026-09-24追加)。
v2/v3との3すくみ比較用: v2(risk_adjusted_blend+pairwise)、v3(exit_class+CrossEntropy)、
v4(exit_class+pairwise、本スクリプト)。特徴量集合(SHORT_STOCK_COLS_V2)は3つとも共通。

実行方法:
    python research/short_exp/backtest_short_model_v4_exitclass_pairwise.py
"""
import sys
import os
...
from train_short_model_v4_exitclass_pairwise import SHORT_MODEL_CACHE_DIR_V4
...
def main():
    ...
    print(f"[+] backtest_pool(空売りv4)={len(pool_test)}")

    models = []
    for seed in m.SEEDS:
        ckpt = os.path.join(SHORT_MODEL_CACHE_DIR_V4, f"state_{seed}.pt")
        if not os.path.exists(ckpt):
            raise FileNotFoundError(
                f"{ckpt}が見つかりません。先にtrain_short_model_v4_exitclass_pairwise.pyを実行してください。"
            )
    ...
    if m.TICKER_TO_CLUSTER:
        print("\n" + "=" * 90)
        print(f"【TOPN別PF分析(空売りモデルv4、クラスタ制約 max_per_cluster={m.MAX_PER_CLUSTER})】")
        print("=" * 90)
